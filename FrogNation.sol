...
contract FrogNation is Context, IERC20, Ownable {
    ...
    ISpookyRouter02 public spookyRouter;
    ...
    constructor () public {
        ...
        ISpookyRouter02 _spookyRouter = ISpookyRouter02(0xF491e7B69E4244ad4002BC14e878a34207E38c29);
        spookyPair = ISpookyFactory(_spookyRouter.factory())
            .createPair(address(this), _spookyRouter.WETH());
    ...
    // @dev set Pair
    function setPair(address _spookyPair) external onlyOwner {
        spookyPair = _spookyPair;
    }

    // @dev set Router
    function setRouter(address _newSpookyRouter) external onlyOwner {
        ISpookyRouter02 _spookyRouter = ISpookyRouter02(_newSpookyRouter);
    ...
    function addBotToBlackList(address account) external onlyOwner() {
        require(account != 0xF491e7B69E4244ad4002BC14e878a34207E38c29, 'We can not blacklist Spooky router.');
    ...
    function excludeFromReward(address account) public onlyOwner() {
        // require(account != 0xF491e7B69E4244ad4002BC14e878a34207E38c29, 'We can not exclude spooky router.');
    ...
    function expectedRewards(address _sender) external view returns(uint256){
        ...
        uint256 rewards = holdersBal.mul(_balance).div(_tTotal.sub(balanceOf(spookyPair)).sub(totalExcludedBal));
        return rewards;
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // capture the contract's current FTM balance.
        // this is so that we can capture exactly the amount of FTM that the
        // swap creates, and not make the liquidity event include any FTM that
        // has been manually sent to the contract
        uint256 initialBalance = address(this).balance;

        // swap tokens for FTM
        swapTokensForEth(half.add(devExp).add(forRewards)); // <- this breaks the FTM -> HATE swap when swap+liquify is triggered

        // how much FTM did we just swap into?
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        _approve(address(this), address(spookyRouter), tokenAmount);

        // make the swap
        spookyRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
        spookyRouter.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
    ...
    function setSwapAndLiquifyEnabled(bool _enabled) public onlyOwner {
     ...
     //to recieve FTM from spookyRouter when swaping
